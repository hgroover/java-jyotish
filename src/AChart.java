...
public class AChart extends Applet implements Runnable
{
	...
	//--------------------------------------------------------------------------
        static boolean m_fStandAlone = false;
	...
	{
                AChart applet_AChart = null;
                m_fStandAlone = true;
                m_fTextOnly = true;
                try {
                    applet_AChart = new AChart();
                }
	...
	*****/
	...
	{
		return "Name: JavaJyotish " + m_Version + "\r\n" +
	...
	public String[][] getParameterInfo()
	{
		String[][] info =
		{
			...
			{ PARAM_Debug, "int", "1 to debug applet" },
                        { "HtmlDivs", "String", "Comma-separated list of chart divisions to display in HTML5" },
		};
		return info;
	}
	...
	public void init2()
	{
		...
		if (!m_fStandAlone) {
			...
			if (param != null) m_PostTarget = param;
                        param = getParameter( "HtmlDivs" );
                        if (param != null) m_htmlDivs = param;
		...
		// If running standalone, parms passed on command line
			...
			for (int i = 0; i < m_args.length; i++) {
				...
				if (s1.equalsIgnoreCase( "TextOnly" )) { m_fTextOnly = true; continue; }
                                if (s1.equalsIgnoreCase( "HtmlDivs" )) { m_htmlDivs = s2; continue; }
...
//	}
	...
	private void Calculate()
	{
		...
		// Get coded output for dRkAna and navAMza if doing rasi chart
		String sAddition = "";
		if (m_nDivision == 1)
		{
			debugOut( "Calculating dRkAna" );
			sAddition += " ";
			for (int i = amath_const.PT_FIRST; i <= amath_const.PT_LAST; i++)
			{
				m_Points[i].RecalcAspects( 3, m_Points );
				sAddition += m_Points[i].GetSymbolic( 3, 1, m_am.GetTithi(), m_am.GetPaksa() );
				sAddition += " ";
			}
