...
// Version: 2.2.2.3 Henry Groover: added persistent day option (keeps selected day if possible when changing month or year)
//  and refocus disable option (do not set focus back to source field - works around IE problem when hiding source field)
...
// 2.2.2.3 note: selDate is constructed when a date is clicked. Changing the month or year does not update unless g_persistDate != 0
...
var g_refocus = 0; // hgroover: set focus back to input control
var g_persistDate = 1; // hgroover: try to keep selected date even if month and/or year is changed
...
Calendar.prototype.SwitchMth = function (intMth)
{
    ...
    var oldMonth = Cal.Month;
	Cal.Month = parseInt(intMth, 10);
	//dbgOut( '<p>SwitchMth(' + intMth + ' old=' + oldMonth + '-' + oldDate + ' new =' + Cal.Month + '-' + Cal.Date + '</p>\n' );
};

Calendar.prototype.SwitchYear = function (intYear)
{
    var oldYear = Cal.Year;
    var oldMonth = Cal.Month;
    var oldDate = Cal.Date;
	Cal.Year = parseInt(intYear, 10);
	//dbgOut( '<p>SwitchYear(' + intYear + ' old=' + oldYear + '-' + oldMonth + '-' + oldDate + ' new=' + Cal.Year + '-' + Cal.Month + '-' + Cal.Date + '</p>\n' );
};
...
function RenderCssCal(bNewCal)
{
    ...
    //dbgOut( 'RenderCssCal('+ bNewCal + ') y=' + Cal.Year + ' m=' + Cal.Month + ' d=' + Cal.Date + ' selDate=' + selDate );
	...
	for (j = 1; j <= Cal.GetMonDays(); j += 1)
	{
		...
		{
            var showCurrentSelection = (j === selDate.getDate());
            if (showCurrentSelection && g_persistDate == 0)
            {
                showCurrentSelection = (Cal.Month === selDate.getMonth() && Cal.Year === selDate.getFullYear());
            }
            else if (showCurrentSelection && (Cal.Month !== selDate.getMonth() || Cal.Year !== selDate.getFullYear()))
            {
                // Modify selected date
                selDate.setMonth(Cal.Month);
                selDate.setFullYear(Cal.Year);
            }
	...
	if (g_refocus) funcCalback += "CalId.focus();";
